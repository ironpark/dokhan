...
pub(crate) fn extract_headwords_preview_impl(
    ...
    let sample_limit = sample_limit.unwrap_or(20).max(1).min(100);
    ...
    for i in 0..total {
        ...
        if lower.ends_with("master.chm") {
            if let Ok(mut chm) = chm::ChmArchive::open(bytes.clone()) {
                if let Ok(hhc) = chm.read_object("master.hhc") {
...
pub(crate) fn preview_chm_paths_impl(
    ...
    let resolved_path = resolve_or_default_zip(zip_path)?;
    let mut archive = open_zip_archive(&resolved_path)?;
    let sample_limit = sample_limit.unwrap_or(10).max(1).min(50);
...
pub(crate) fn get_content_page_impl(
    ...
    match &source {
        RuntimeSource::DebugRoot(root) => {
            let path = root.join("master").join(&local);
            if !path.exists() {
                return Err(format!("content page not found: {}", path.display()));
            }
            let text = read_text_auto(&path)?;
        ...
        RuntimeSource::ZipPath(_) => {
            let runtime = get_runtime(&source)?;
            runtime
                .content_pages
                .get(&local)
                .cloned()
                .ok_or_else(|| format!("content page not found in zip runtime: {local}"))
        }
    }
}
